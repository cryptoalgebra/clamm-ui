...
export function ALMTag({ poolAddress }: { poolAddress: Address }) {
    ...
    if (isLoading || !vaults || vaults.length === 0) {
        return (
            <div className="flex h-[26px] w-fit items-center justify-center rounded-full bg-alm px-3 py-1 text-xs font-bold text-text-100 shadow-inner shadow-white duration-200 hover:opacity-80 max-md:text-xs">
                ALM
            </div>
        );
    }
    ...
    return (
        <HoverCard openDelay={100} closeDelay={100}>
            <HoverCardTrigger asChild>
                <div className="flex h-[26px] w-fit cursor-pointer text-text-100 items-center justify-center rounded-full bg-alm px-3 py-1 text-xs font-bold shadow-inner shadow-white duration-200 hover:opacity-80 max-md:text-xs">
                    ALM
                </div>
            </HoverCardTrigger>
